...
fun asBackupFailedData(backupFailedData: ReadableMap): BackupFailedData? {
    if (!validateMandatoryFields(backupFailedData, arrayOf(
            "error",
    ))) {
        return null
    }
...
fun asBackupStatus(backupStatus: ReadableMap): BackupStatus? {
    if (!validateMandatoryFields(backupStatus, arrayOf(
            "backedUp",
    ))) {
        return null
    }
...
fun asBuyBitcoinRequest(buyBitcoinRequest: ReadableMap): BuyBitcoinRequest? {
    ...
    val openingFeeParams = if (hasNonNullKey(buyBitcoinRequest, "openingFeeParams")) buyBitcoinRequest.getMap("openingFeeParams")?.let { asOpeningFeeParams(it)} else null
...
fun asBuyBitcoinResponse(buyBitcoinResponse: ReadableMap): BuyBitcoinResponse? {
    if (!validateMandatoryFields(buyBitcoinResponse, arrayOf(
            "url",
    ))) {
        return null
    }
    val url = buyBitcoinResponse.getString("url")!!
    val openingFeeParams = if (hasNonNullKey(buyBitcoinResponse, "openingFeeParams")) buyBitcoinResponse.getMap("openingFeeParams")?.let { asOpeningFeeParams(it)} else null
...
fun asClosedChannelPaymentDetails(closedChannelPaymentDetails: ReadableMap): ClosedChannelPaymentDetails? {
    if (!validateMandatoryFields(closedChannelPaymentDetails, arrayOf(
            "state",
            "fundingTxid",
    ))) {
        return null
    ...
    val shortChannelId = if (hasNonNullKey(closedChannelPaymentDetails, "shortChannelId")) closedChannelPaymentDetails.getString("shortChannelId") else null
    val closingTxid = if (hasNonNullKey(closedChannelPaymentDetails, "closingTxid")) closedChannelPaymentDetails.getString("closingTxid") else null
...
fun asConfigureNodeRequest(configureNodeRequest: ReadableMap): ConfigureNodeRequest? {
    if (!validateMandatoryFields(configureNodeRequest, arrayOf(
    ))) {
        return null
    }
    val closeToAddress = if (hasNonNullKey(configureNodeRequest, "closeToAddress")) configureNodeRequest.getString("closeToAddress") else null
...
fun asGreenlightCredentials(greenlightCredentials: ReadableMap): GreenlightCredentials? {
    if (!validateMandatoryFields(greenlightCredentials, arrayOf(
            "developerKey",
            "developerCert",
    ))) {
        return null
...
fun asGreenlightDeviceCredentials(greenlightDeviceCredentials: ReadableMap): GreenlightDeviceCredentials? {
    if (!validateMandatoryFields(greenlightDeviceCredentials, arrayOf(
            "device",
    ))) {
        return null
    }
...
fun asGreenlightNodeConfig(greenlightNodeConfig: ReadableMap): GreenlightNodeConfig? {
    if (!validateMandatoryFields(greenlightNodeConfig, arrayOf(
    ))) {
        return null
    }
    val partnerCredentials = if (hasNonNullKey(greenlightNodeConfig, "partnerCredentials")) greenlightNodeConfig.getMap("partnerCredentials")?.let { asGreenlightCredentials(it)} else null
...
fun asInvoicePaidDetails(invoicePaidDetails: ReadableMap): InvoicePaidDetails? {
    if (!validateMandatoryFields(invoicePaidDetails, arrayOf(
            "paymentHash",
            "bolt11",
    ))) {
        return null
...
fun asListPaymentsRequest(listPaymentsRequest: ReadableMap): ListPaymentsRequest? {
    ...
    val filters = if (hasNonNullKey(listPaymentsRequest, "filters")) listPaymentsRequest.getArray("filters")?.let { asPaymentTypeFilterList(it) } else null
    val metadataFilters = if (hasNonNullKey(listPaymentsRequest, "metadataFilters")) listPaymentsRequest.getArray("metadataFilters")?.let { asMetadataFilterList(it) } else null
    ...
    val includeFailures = if (hasNonNullKey(listPaymentsRequest, "includeFailures")) listPaymentsRequest.getBoolean("includeFailures") else null
...
fun asLnPaymentDetails(lnPaymentDetails: ReadableMap): LnPaymentDetails? {
    if (!validateMandatoryFields(lnPaymentDetails, arrayOf(
            "paymentHash",
            "label",
            "destinationPubkey",
            "paymentPreimage",
    ...
    val openChannelBolt11 = if (hasNonNullKey(lnPaymentDetails, "openChannelBolt11")) lnPaymentDetails.getString("openChannelBolt11") else null
    val lnurlSuccessAction = if (hasNonNullKey(lnPaymentDetails, "lnurlSuccessAction")) lnPaymentDetails.getMap("lnurlSuccessAction")?.let { asSuccessActionProcessed(it)} else null
    ...
    val lnurlWithdrawEndpoint = if (hasNonNullKey(lnPaymentDetails, "lnurlWithdrawEndpoint")) lnPaymentDetails.getString("lnurlWithdrawEndpoint") else null
    val swapInfo = if (hasNonNullKey(lnPaymentDetails, "swapInfo")) lnPaymentDetails.getMap("swapInfo")?.let { asSwapInfo(it)} else null
    val reverseSwapInfo = if (hasNonNullKey(lnPaymentDetails, "reverseSwapInfo")) lnPaymentDetails.getMap("reverseSwapInfo")?.let { asReverseSwapInfo(it)} else null
    val pendingExpirationBlock = if (hasNonNullKey(lnPaymentDetails, "pendingExpirationBlock")) lnPaymentDetails.getInt("pendingExpirationBlock").toUInt() else null
...
fun asLspInformation(lspInformation: ReadableMap): LspInformation? {
    if (!validateMandatoryFields(lspInformation, arrayOf(
            "id",
            "name",
            "widgetUrl",
            "pubkey",
...
fun asMaxReverseSwapAmountResponse(maxReverseSwapAmountResponse: ReadableMap): MaxReverseSwapAmountResponse? {
    if (!validateMandatoryFields(maxReverseSwapAmountResponse, arrayOf(
            "totalSat",
    ))) {
        return null
    }
...
fun asMetadataFilter(metadataFilter: ReadableMap): MetadataFilter? {
    if (!validateMandatoryFields(metadataFilter, arrayOf(
            "jsonPath",
            "jsonValue",
    ))) {
        return null
...
fun asMetadataItem(metadataItem: ReadableMap): MetadataItem? {
    if (!validateMandatoryFields(metadataItem, arrayOf(
            "key",
            "value",
    ))) {
        return null
...
fun asNodeState(nodeState: ReadableMap): NodeState? {
    if (!validateMandatoryFields(nodeState, arrayOf(
            "id",
            "blockHeight",
            "channelsBalanceMsat",
            "onchainBalanceMsat",
...
fun asOpenChannelFeeRequest(openChannelFeeRequest: ReadableMap): OpenChannelFeeRequest? {
    if (!validateMandatoryFields(openChannelFeeRequest, arrayOf(
    ))) {
        return null
    }
    val amountMsat = if (hasNonNullKey(openChannelFeeRequest, "amountMsat")) openChannelFeeRequest.getDouble("amountMsat").toULong() else null
...
fun asOpenChannelFeeResponse(openChannelFeeResponse: ReadableMap): OpenChannelFeeResponse? {
    if (!validateMandatoryFields(openChannelFeeResponse, arrayOf(
            "feeParams",
    ))) {
        return null
    }
...
fun asOpeningFeeParams(openingFeeParams: ReadableMap): OpeningFeeParams? {
    if (!validateMandatoryFields(openingFeeParams, arrayOf(
            "minMsat",
            "proportional",
            "validUntil",
            "maxIdleTime",
...
fun asOpeningFeeParamsMenu(openingFeeParamsMenu: ReadableMap): OpeningFeeParamsMenu? {
    if (!validateMandatoryFields(openingFeeParamsMenu, arrayOf(
            "values",
    ))) {
        return null
    }
...
fun asPayOnchainResponse(payOnchainResponse: ReadableMap): PayOnchainResponse? {
    if (!validateMandatoryFields(payOnchainResponse, arrayOf(
            "reverseSwapInfo",
    ))) {
        return null
    }
...
fun asPaymentFailedData(paymentFailedData: ReadableMap): PaymentFailedData? {
    if (!validateMandatoryFields(paymentFailedData, arrayOf(
            "error",
            "nodeId",
    ))) {
        return null
...
fun asPrepareOnchainPaymentRequest(prepareOnchainPaymentRequest: ReadableMap): PrepareOnchainPaymentRequest? {
    if (!validateMandatoryFields(prepareOnchainPaymentRequest, arrayOf(
            "amountSat",
            "amountType",
            "claimTxFeerate",
    ))) {
...
fun asPrepareOnchainPaymentResponse(prepareOnchainPaymentResponse: ReadableMap): PrepareOnchainPaymentResponse? {
    if (!validateMandatoryFields(prepareOnchainPaymentResponse, arrayOf(
            "feesHash",
            "feesPercentage",
            "feesLockup",
            "feesClaim",
...
fun asPrepareRedeemOnchainFundsRequest(prepareRedeemOnchainFundsRequest: ReadableMap): PrepareRedeemOnchainFundsRequest? {
    if (!validateMandatoryFields(prepareRedeemOnchainFundsRequest, arrayOf(
            "toAddress",
            "satPerVbyte",
    ))) {
        return null
...
fun asPrepareRedeemOnchainFundsResponse(prepareRedeemOnchainFundsResponse: ReadableMap): PrepareRedeemOnchainFundsResponse? {
    if (!validateMandatoryFields(prepareRedeemOnchainFundsResponse, arrayOf(
            "txWeight",
            "txFeeSat",
    ))) {
        return null
...
fun asReceiveOnchainRequest(receiveOnchainRequest: ReadableMap): ReceiveOnchainRequest? {
    if (!validateMandatoryFields(receiveOnchainRequest, arrayOf(
    ))) {
        return null
    }
    val openingFeeParams = if (hasNonNullKey(receiveOnchainRequest, "openingFeeParams")) receiveOnchainRequest.getMap("openingFeeParams")?.let { asOpeningFeeParams(it)} else null
...
fun asReceivePaymentRequest(receivePaymentRequest: ReadableMap): ReceivePaymentRequest? {
    ...
    val preimage = if (hasNonNullKey(receivePaymentRequest, "preimage")) receivePaymentRequest.getArray("preimage")?.let { asUByteList(it) } else null
    val openingFeeParams = if (hasNonNullKey(receivePaymentRequest, "openingFeeParams")) receivePaymentRequest.getMap("openingFeeParams")?.let { asOpeningFeeParams(it)} else null
...
fun asReceivePaymentResponse(receivePaymentResponse: ReadableMap): ReceivePaymentResponse? {
    ...
    val openingFeeParams = if (hasNonNullKey(receivePaymentResponse, "openingFeeParams")) receivePaymentResponse.getMap("openingFeeParams")?.let { asOpeningFeeParams(it)} else null
    val openingFeeMsat = if (hasNonNullKey(receivePaymentResponse, "openingFeeMsat")) receivePaymentResponse.getDouble("openingFeeMsat").toULong() else null
...
fun asRedeemOnchainFundsRequest(redeemOnchainFundsRequest: ReadableMap): RedeemOnchainFundsRequest? {
    if (!validateMandatoryFields(redeemOnchainFundsRequest, arrayOf(
            "toAddress",
            "satPerVbyte",
    ))) {
        return null
...
fun asRedeemOnchainFundsResponse(redeemOnchainFundsResponse: ReadableMap): RedeemOnchainFundsResponse? {
    if (!validateMandatoryFields(redeemOnchainFundsResponse, arrayOf(
            "txid",
    ))) {
        return null
    }
...
fun asReportPaymentFailureDetails(reportPaymentFailureDetails: ReadableMap): ReportPaymentFailureDetails? {
    if (!validateMandatoryFields(reportPaymentFailureDetails, arrayOf(
            "paymentHash",
    ))) {
        return null
    }
...
fun asReverseSwapFeesRequest(reverseSwapFeesRequest: ReadableMap): ReverseSwapFeesRequest? {
    if (!validateMandatoryFields(reverseSwapFeesRequest, arrayOf(
    ))) {
        return null
    }
    val sendAmountSat = if (hasNonNullKey(reverseSwapFeesRequest, "sendAmountSat")) reverseSwapFeesRequest.getDouble("sendAmountSat").toULong() else null
    val claimTxFeerate = if (hasNonNullKey(reverseSwapFeesRequest, "claimTxFeerate")) reverseSwapFeesRequest.getInt("claimTxFeerate").toUInt() else null
...
fun asReverseSwapInfo(reverseSwapInfo: ReadableMap): ReverseSwapInfo? {
    if (!validateMandatoryFields(reverseSwapInfo, arrayOf(
            "id",
            "claimPubkey",
            "onchainAmountSat",
            "status",
...
fun asReverseSwapPairInfo(reverseSwapPairInfo: ReadableMap): ReverseSwapPairInfo? {
    if (!validateMandatoryFields(reverseSwapPairInfo, arrayOf(
            "min",
            "max",
            "feesHash",
            "feesPercentage",
...
fun asSendOnchainRequest(sendOnchainRequest: ReadableMap): SendOnchainRequest? {
    if (!validateMandatoryFields(sendOnchainRequest, arrayOf(
            "amountSat",
...
fun asSendOnchainResponse(sendOnchainResponse: ReadableMap): SendOnchainResponse? {
    if (!validateMandatoryFields(sendOnchainResponse, arrayOf(
            "reverseSwapInfo",
    ))) {
        return null
    }
...
fun asSendSpontaneousPaymentRequest(sendSpontaneousPaymentRequest: ReadableMap): SendSpontaneousPaymentRequest? {
    if (!validateMandatoryFields(sendSpontaneousPaymentRequest, arrayOf(
            "nodeId",
            "amountMsat",
    ))) {
        return null
    ...
    val extraTlvs = if (hasNonNullKey(sendSpontaneousPaymentRequest, "extraTlvs")) sendSpontaneousPaymentRequest.getArray("extraTlvs")?.let { asTlvEntryList(it) } else null
...
fun asServiceHealthCheckResponse(serviceHealthCheckResponse: ReadableMap): ServiceHealthCheckResponse? {
    if (!validateMandatoryFields(serviceHealthCheckResponse, arrayOf(
            "status",
    ))) {
        return null
    }
...
fun asStaticBackupRequest(staticBackupRequest: ReadableMap): StaticBackupRequest? {
    if (!validateMandatoryFields(staticBackupRequest, arrayOf(
            "workingDir",
    ))) {
        return null
    }
...
fun asStaticBackupResponse(staticBackupResponse: ReadableMap): StaticBackupResponse? {
    if (!validateMandatoryFields(staticBackupResponse, arrayOf(
    ))) {
        return null
    }
    val backup = if (hasNonNullKey(staticBackupResponse, "backup")) staticBackupResponse.getArray("backup")?.let { asStringList(it) } else null
...
fun asSwapInfo(swapInfo: ReadableMap): SwapInfo? {
    if (!validateMandatoryFields(swapInfo, arrayOf(
            "bitcoinAddress",
            "createdAt",
            "lockHeight",
            "paymentHash",
    ...
    val channelOpeningFees = if (hasNonNullKey(swapInfo, "channelOpeningFees")) swapInfo.getMap("channelOpeningFees")?.let { asOpeningFeeParams(it)} else null
    val confirmedAt = if (hasNonNullKey(swapInfo, "confirmedAt")) swapInfo.getInt("confirmedAt").toUInt() else null
    val confirmedAtTimestamp = if (hasNonNullKey(swapInfo, "confirmedAtTimestamp")) swapInfo.getDouble("confirmedAtTimestamp").toULong() else null
    return SwapInfo(
        bitcoinAddress,
        createdAt,
        lockHeight,
...
fun readableMapOf(swapInfo: SwapInfo): ReadableMap {
    return readableMapOf(
            ...
            "confirmedAtTimestamp" to swapInfo.confirmedAtTimestamp,       
    )
}
...
fun asTlvEntry(tlvEntry: ReadableMap): TlvEntry? {
    if (!validateMandatoryFields(tlvEntry, arrayOf(
            "fieldNumber",
            "value",
    ))) {
        return null
...
fun asUnspentTransactionOutput(unspentTransactionOutput: ReadableMap): UnspentTransactionOutput? {
    if (!validateMandatoryFields(unspentTransactionOutput, arrayOf(
            "txid",
            "outnum",
            "amountMillisatoshi",
            "address",
