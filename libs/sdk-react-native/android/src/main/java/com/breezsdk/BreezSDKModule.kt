...
class BreezSDKModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    @ReactMethod
    fun serviceHealthCheck(apiKey: String, promise: Promise) {
        executor.execute {
            try {
    ...
    @ReactMethod
    fun unregisterWebhook(webhookUrl: String, promise: Promise) {
        executor.execute {
            try {
    ...
    @ReactMethod
    fun redeemSwap(swapAddress: String, promise: Promise) {
        executor.execute {
            try {
    ...
    @ReactMethod
    fun prepareOnchainPayment(req: ReadableMap, promise: Promise) {
        executor.execute {
            try {
                val prepareOnchainPaymentRequest = asPrepareOnchainPaymentRequest(req) ?: run { throw SdkException.Generic(errMissingMandatoryField("req", "PrepareOnchainPaymentRequest")) }
    ...
    @ReactMethod
    fun claimReverseSwap(lockupAddress: String, promise: Promise) {
        executor.execute {
            try {
